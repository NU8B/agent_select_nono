# Testing the algorithm with our own benchmark
# Accuracy: 66/69 (95.7%)
...
import functools # Cache results for efficiency
...
def load_agents() -> List:
    ...
    agents = []  # Create an empty list to store agent objects
    for filename in os.listdir("data/agents"):  # Loop through all the agent
        if filename.endswith(".json"):  # Check if the file is a JSON file
            with open(os.path.join("data/agents", filename)) as file:  # Open the JSON file
                data = json.load(file)  # Load the JSON data into a dictionary
                # Create a dynamic 'Agent' object with attributes from the JSON data
                agent = type(
                    "Agent",
                    (),
                    # Select the attributes from the JSON data
                    {
                        "name": data.get("name"),
                ...
                agents.append(agent)  # Add the agent object to the list
    return agents  # Return the list of agent objects
...
class AgentCache:
    ...
    def initialize(self, agents: List) -> None:
        ...
        # Create lookup dictionary to quickly look up agents by their description
        self.agent_lookup = {agent.description: agent for agent in agents}

        # Process agent value to calculate and store their values
        self.agent_values = {
...
class StellaTestAlgorithm:
    ...
    def _compute_scores(
        ...
        # Extract pre-calculated weights and normalized ratings from agent data
        ...
        # Calculate lexical scores by comparing the query with each agent's description
        ...
        # Calculate the combined score for each agent
        combined_scores = (
        ...
        # Create a list of dictionaries containing detailed scoring information for each agent
        selection_details = [
            {
